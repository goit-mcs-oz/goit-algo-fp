...
# Створення купи
heap = [0, 4, 1, 5, 10, 3, 6, 7]
heapq.heapify(heap)
print(heap)

heap = list(map(lambda item: Node(item), heap))
for index, item in enumerate(heap):
    root_node = heap[index]
    index_l = 2*index + 1
    if index_l <= len(heap) - 1:
        root_node.left = heap[index_l]
    index_r = 2*index + 2
    if index_r <= len(heap) - 1:
        root_node.right = heap[index_r]
